...
from training.env import TorcsEnv
from training.networks.actor import Actor
from training.networks.critic import Critic
from training.agent import A2CAgent
...
def train():
    ...
    actor = Actor(obs_dim=30, action_dim=2)
    critic = Critic(obs_dim=30)
    ...
    for episode in range(start_ep, 10000):
        ...
        while not done:
            ...
            if step % 50 == 0:
                S = env.client.S.d
                print(f"[STEER] trackPos={S.get('trackPos',0):.3f} "
            ...
            if step == 100:
                S = env.client.S.d
                print(f"[STEP100] speed={S.get('speedX',0):.1f} "
                    f"gear={int(S.get('gear',1))} "
                    f"dist={S.get('distRaced',0):.1f} "
