...
def drive_modular(c):
    ...
    if dist_bounds < 50 : # start line to turn 1
        drive_to_speed(S, R, 70, 0.3)
    if dist_bounds >= 50 and dist_bounds < 180:
    ...
    if dist_bounds > 230 and dist_bounds < 260: # straight to turn 2 
        turn_to_corner(S, R, -0.8, 0.01) # in this instance the car turns to be -80% left of the centre of the track (80% to the right)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 260 and dist_bounds <= 390: # straight to turn 2 prep
        turn_within_lines(S, R, -0.7, -0.9, 0.009)
        drive_to_speed(S, R, 90, 0.6) # in this instance the car aims to drive at a limit of 90km/h and brakes with 50% power
    ...
    if dist_bounds >= 970 and dist_bounds < 1050: # turn 4 code
        turn_within_lines(S, R, -0.75, -0.99, 0.05)
        drive_to_speed(S, R, 90, 0.3)
    if dist_bounds >= 1050 and dist_bounds < 1425: # straight to turn 5
        turn_within_lines(S, R, -0.4, -0.7, 0.02)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    ...
    if dist_bounds >= 2850 and dist_bounds < 2920: # turn 10 prep
        turn_within_lines(S, R, 0.99, 0.75, 0.005)
        drive_to_speed(S, R, 120, 0.3)
    if dist_bounds >= 2920 and dist_bounds < 3000: # turn 10 code
        turn_within_lines(S, R, -0.75, -0.99, 0.035)
