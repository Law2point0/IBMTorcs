...
def drive_modular(c):
    ...
    if dist_bounds >= 300 and dist_bounds <= 390: # straight to turn 2 prep
        ...
        drive_to_speed(S, R, 90, 0.4) # in this instance the car aims to drive at a limit of 100km/h and brakes with 40% power
    if dist_bounds > 390 and dist_bounds < 530: # turn 2 code
        turn_within_lines(S, R, 0.99, 0.75, 0.075)
        drive_to_speed(S, R, 80, 0.25)
    if dist_bounds >= 530 and dist_bounds <= 600: # kink to turn 3
        turn_within_lines(S, R, -0.5, -0.85, 0.03)
    ...
    if dist_bounds >= 790 and dist_bounds < 970: # straight to turn 4
        turn_within_lines(S, R, 0.99, 0.2, 0.01)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 970 and dist_bounds < 1050: # turn 4 code
        turn_within_lines(S, R, -0.75, -0.99, 0.04)
        drive_to_speed(S, R, 115, 0.3)
    if dist_bounds >= 1050 and dist_bounds < 1425: # straight to turn 5
        turn_within_lines(S, R, -0.1, -0.9, 0.0075)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 1425 and dist_bounds < 1450: # turn 5 prep
        turn_within_lines(S, R, -0.5, -0.8, 0.01)
        drive_to_speed(S, R, 90, 0.32)
    if dist_bounds >= 1450 and dist_bounds < 1575: # turn 5 code
        turn_within_lines(S, R, 1.02, 0.75, 0.05)
        drive_to_speed(S, R, 95, 0.3)
    if dist_bounds >= 1575 and dist_bounds < 1800: # straight to turn 6
        turn_to_corner(S, R, -0.8, 0.005)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 1850 and dist_bounds < 1885: # turn 6 prep
        turn_within_lines(S, R, -0.6, -0.8, 0.01)
        drive_to_speed(S, R, 110, 0.3)
    if dist_bounds >= 1885 and dist_bounds < 1950: # turn 6 code
        turn_within_lines(S, R, 0.99, 0.75, 0.06)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 1950 and dist_bounds < 2260: # straight to turn 7/8 (corkscrew)
        turn_to_corner(S, R, -0.8, 0.001)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 2260 and dist_bounds < 2320: # straight to turn 7/8 (corkscrew)
        turn_within_lines(S, R, -0.4, -0.9, 0.015)
        drive_to_speed(S, R, 100, 0.3)
    if dist_bounds >= 2320 and dist_bounds < 2425: # prep for turn 7/8 (corkscrew)
        turn_within_lines(S, R, -0.2, -0.7, 0.075)
    ...
    if dist_bounds >= 2484 and dist_bounds < 2515: # turn 8 code
        ...
        drive_to_speed(S, R, 30, 0.2)
    if dist_bounds >= 2515 and dist_bounds < 2650: # straight to turn 9
        turn_within_lines(S, R, -0.6, -0.99, 0.01)
        drive_to_speed(S, R, 120, 0.3)
    if dist_bounds >= 2650 and dist_bounds < 2800: # turn 9 code
        turn_within_lines(S, R, 0.99, 0.6, 0.045)
        drive_to_speed(S, R, 135, 0.3)
    if dist_bounds >= 2800 and dist_bounds < 2850: # straight to turn 10
        turn_to_corner(S, R, 0.8, 0.01)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 2850 and dist_bounds < 2910: # turn 10 prep
        turn_within_lines(S, R, 0.99, 0.85, 0.015)
        drive_to_speed(S, R, 120, 0.3)
    if dist_bounds >= 2910 and dist_bounds < 3000: # turn 10 code
        turn_within_lines(S, R, -0.75, -0.99, 0.045)
        drive_to_speed(S, R, 120, 0.3)
    if dist_bounds >= 3000 and dist_bounds < 3120: # straight to turn 11
        turn_to_corner(S, R, -0.8, 0.005)
        drive_to_speed(S, R, TARGET_SPEED, 0.3)
    if dist_bounds >= 3120 and dist_bounds < 3235: # turn 11 prep
        turn_within_lines(S, R, -0.99, -0.75, 0.01)
        drive_to_speed(S, R, 50, 0.3)
    if dist_bounds >= 3235 and dist_bounds < 3275: # turn 11 code
        turn_within_lines(S, R, 1.1, 0.75, 0.075)
        drive_to_speed(S, R, 35, 0.3)
