...
class Agent(object):
    ...
    def __init__(self, dim_action):
        self.dim_action = dim_action
        self.file = open('telemetry.csv', 'w', newline='')
        self.writer = csv.writer(self.file)
        self.writer.writerow(['speedX', 'speedY', 'speedZ', 'rpm'])
    ...
    def act(self, ob, reward, done, vision_on):
        ...
        # Log selected telemetry (lightweight logging)
        self.writer.writerow([speedX, speedY, speedZ, rpm])

        # Close file safely when episode ends
        if done:
            self.file.close()
