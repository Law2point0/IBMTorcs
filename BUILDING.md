...
## Installation
1. Move all files/folders from within `code/` into `gym_torcs/`
2. Install dependencies with pip by running `pip install -r requirements` in the project root. You may need to prefix this with `python -m ` (Windows) or `python3 -m ` (MacOS/Linux) if pip isn't a recognized command. You can optionally create a virtual environment if you don't want to install these dependencies globally.
3. (Recommended) Download Granite Micro 4.0 through Ollama by running `ollama pull hf.co/ibm-granite/granite-4.0-micro-GGUF:Q4_K_M'`. This is so it doesn't download the model on first prompt instead.

## Running
1. Ensure Ollama's background task is running either by opening and closing it or checking if it's running in system tray.
2. Open a terminal in the `gym_torcs/` directory.
